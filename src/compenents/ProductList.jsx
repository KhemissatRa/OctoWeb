...
export default function ProductList() {
  ...
  {products.map((product) => (
      ...
      <div key={product.id} className="border border-gray-500/20 rounded-md px-3 md:px-4 py-2 bg-white w-full">
    <div className="group cursor-pointer flex items-center justify-center px-2">
        <img className="group-hover:scale-105 transition max-w-[160px]" src={product.Image} alt="Product Image" />
    </div>
    ...
    <div className="text-gray-500/60 text-sm ">
        ...
        <div className="flex items-end justify-between mt-2">
            <p className="md:text-xl text-base font-medium text-indigo-500">
                {product.price} DZD <span className="text-gray-500/60 md:text-sm text-xs line-through">{product.price + 21}DZD</span>
            </p>
            <button className="flex  bg-blue-600 items-center justify-center gap-1 bg-indigo-100 border border-indigo-300 md:w-[80px] w-[64px] h-[34px] rounded text-indigo-600 font-medium">
                ...
                <Link to={`/productDetais/${product._id}`}>Buy</Link>
            </button>
        </div>
    </div>
</div>)
